...
class LoginForm extends DbConn
{
    public function checkLogin($myusername, $mypassword)
    {
        ...
        if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
            ...
            $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\">&times;</button>Maximum number of login attempts exceeded. Please wait ".$timeout_minutes." minutes before trying again</div>";

        } else {

		// Check Time (Disabled From 10-5)
		$time = date("H");
		if ($time >= 22 || $time <= 4) {
			$success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\">&times;</button>Service shutdown. Try again tomorrow.</div>";
		} elseif (password_verify($mypassword, $result['password']) && $result['verified'] == '1') {
		//Success! Register $myusername, $mypassword and return "true"
		$success = 'true';
		session_start();
		
		$_SESSION['username'] = $myusername;
		$_SESSION['AuthLevel'] = $result['AuthLevel'];
		...
		$_SESSION['id'] = $result["id"];
			...
			$success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\">&times;</button>Your account is currently disabled</div>";	
		} else {
		
		//Wrong username or password
		$success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\">&times;</button>Wrong Username or Password</div>";
